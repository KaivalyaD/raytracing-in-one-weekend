...
int main(void) {
    ...
    auto material_blue = make_shared<lambertian>(color3(0.0f, 0.0f, 1.0f));
    auto material_red = make_shared<lambertian>(color3(1.0f, 0.0f, 0.0f));

    world.add(make_shared<sphere>(point3(-R, 0.0f, -1.0f), R, material_blue));
    world.add(make_shared<sphere>(point3( R, 0.0f, -1.0f), R, material_red));

    // camera
    camera cam = camera(132.5f, aspect_ratio);
